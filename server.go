...
func saveMessage(from, to, text, imageData, sticker string, isGroup bool, groupID string) {
	...
	if err != nil {
		...
		fmt.Println("Сообщение сохранено успешно")
	}
}
...
func getPrivateHistory(u1, u2 string) []Message {
	...
	if err != nil {
		fmt.Println("Ошибка истории:", err)
		return nil
	}
	defer rows.Close()
	var msgs []Message
...
func getGroupHistory(groupID string) []Message {
	...
	if err != nil {
		fmt.Println("Ошибка истории группы:", err)
		return nil
	}
	defer rows.Close()
	var msgs []Message
...
func handleWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		fmt.Printf("[WS] Получено: type=%s, from=%s, to=%s, text=%s\n", msg.Type, msg.From, msg.To, msg.Text)
...
func handleChat(client *Client) {
	...
	for {
		...
		fmt.Printf("[CHAT] Получено: type=%s, to=%s, text=%s, isGroup=%v\n", msg.Type, msg.To, msg.Text, client.IsGroup)

		switch msg.Type {
		case "select_dialog":
			client.CurrentDialog = msg.To
			client.IsGroup = false
			fmt.Printf("[CHAT] Выбран диалог с %s\n", msg.To)
		...
		case "select_group":
			client.CurrentDialog = msg.To
			client.IsGroup = true
			fmt.Printf("[CHAT] Выбрана группа %s\n", msg.To)
		...
		case "message":
			fmt.Printf("[CHAT] Обработка сообщения: isGroup=%v, CurrentDialog=%s\n", client.IsGroup, client.CurrentDialog)
			if client.IsGroup {
				groupID := client.CurrentDialog
				members := getGroupMembers(groupID)
				saveMessage(client.Name, "", msg.Text, msg.ImageData, msg.Sticker, true, groupID)
				fmt.Printf("[CHAT] Сообщение в группу %s от %s\n", groupID, client.Name)
				mutex.Lock()
				for c := range clients {
				...
				fmt.Printf("[CHAT] Сообщение от %s к %s\n", client.Name, msg.To)
